...
class Lakeshore(object):
    def __init__(self, devfile, TIMEOUT_IGNORE=True):
        ...
        if isinstance(devfile, str):
            ...
            if not('ip' in devfile and 'mask' in devfile and 'port' in devfile):
                raise LakeshoreError(
                    'dictionary keys error in arguments {}'.format(','.join(devfile.keys())))
                pass
            ...
            pass
        if self.dev == None:
            raise LakeshoreError("open error")
    ...
    def write(self, command):
        if self.devtype == 'serial':
            self.dev.write(command.encode() + b'\n')
            sleep(0.1)
            pass
        elif self.devtype == 'ethernet':
            self.dev.send(command.encode() + b'\n')
            pass
        pass
    ...
    def gettemps(self, chlist=None, mode="K"):
        ...
        ret = []
        for c in self.chs:
            ret.append(self.gettemp(c, mode))
        return ret
    ...
    def addcalibration(self, ch, name='aho', serialnumber=' ', dataformat=0, templimit=300, posneg=1):
        ...
        if self.devtype == 'serial':
            command = 'CRVHDR 2{},"{}","{}",{},{},{}'.format(
                ch, name, serialnumber, dataformat, modtemplimit, posneg)
            self.write(command)
            sleep(0.1)
            pass
        elif self.devtype == 'ethernet':
    ...
    def getsampleheatermax(self):
        self.readclear()
        self.write('range?')
        intToAmpere = [0., 31.6e-6, 100.e-6, 316.e-6,
                       1.0e-3, 3.16e-3, 10.0e-3, 31.6e-3, 100.e-3]
        ret = self.read()
